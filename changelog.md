...
- **自定义第三方音源平台独立启用/禁用控制 (#292)**:
  - **音源平台编辑面板**: 在“自定义源管理”与设置界面的音源列表中，每个音源卡片的删除按钮左侧新增「编辑」按钮，点击弹出高层级音源平台编辑面板。
  - **平台精准开闭与状态持久化**: 支持单独为该音源开启或关闭特定平台（如酷我、咪咕、酷狗、QQ、网易等）的解析支持，配置持久化记录于 `states.json` 中；搜索与播放时若某平台被关闭，系统将自动跳过对应平台的歌曲解析。
  - **卡片徽章联动与一键快捷操作**: 音源卡片上的支持平台徽章实时联动，被禁用的平台自动呈现半透明与删除线样式；编辑面板内支持“全启用”与“全关闭”一键批量操作。
  - ![自定义音源平台编辑面板](md/log/2.0.1-2.png)
- **换源与降级试错常驻状态卡片**:
  - **单例常驻交互**: 解决歌曲播放失败、换源试错与音质降级过程中弹出大量重叠碎屑 Toast 的问题，改用单个常驻 Toast 实时跟踪显示换源与降级试错轨迹。
  - **明亮 / 暗黑与多主题自适应**: 重构卡片视觉体系，完美适配明亮模式（高透毛玻璃白底）与暗黑模式，并与系统 5 款动态主题风格（森之韵、深海谧、暖阳愿、紫微星、绯红月）实时联动变色。
  - **过程消息收敛与智能避让**: 试错换源期间自动收敛后端 SSE 进度等过程性成功/失败通知，避免底层重合；其它独立业务 Toast 自动向上位移避让，不再重叠遮挡。
  - ![换源失败与试错常驻提示](md/log/2.0.1-3.png)
- **服务端存储目录与层级透明化查看 (#302)**:
  - **一键查看物理绝对路径**: 在设置中心「歌曲命名方式」下方新增「查看当前存储目录」功能，点击即可查看当前操作系统底层真实的物理绝对路径（如 Windows 下的 `E:\...\` 或 Linux/Docker 物理路径）。
  - **综合决策与模式状态解析**: 综合计算当前用户的「缓存歌曲位置」（`DATA_PATH (WebDAV同步)` 与 `运行目录 (仅本地)`）与「仅下载模式」（开启则下载与缓存目录独立分离，关闭则合并归属），并清晰展示所属用户空间。
  - **路径快速复制与规则指引**: 分别提供歌曲缓存目录与歌曲下载目录的完整路径，支持一键复制到剪贴板，并配有详细的备份边界与洗版范围说明，彻底解决目录层级不清的问题。
  - ![当前存储目录面板](md/log/2.0.1-4.png)
- **Subsonic 歌单管理接口补全 (#311)**:
  - **歌单增删改支持**: 实现 `createPlaylist`、`deletePlaylist`，并补全 `updatePlaylist` 的歌曲添加、删除、改名及位置调整能力；兼容重复参数、逗号分隔参数与 `songIdToAdd`、`songIndexToAdd` 等常用客户端调用方式。
  - **数据持久化与添加位置**: Subsonic 歌单变更后自动创建快照，并统一遵循用户的 `LIST_ADD_MUSIC_LOCATION_TYPE` 配置。
- **Subsonic 多平台搜索增强**:
  - **搜索范围选择**: 音乐目录新增本地曲库、全部在线平台及网易云、QQ、酷我、酷狗、咪咕等独立平台入口；关键词支持 `all:` / `online:`、`local:` 及各平台前缀搜索。
  - **稳定分页结果**: 对多平台结果进行交错合并与短期缓存，避免客户端翻页时出现重复、跳项或搜索结果不全。
- **Web 播放器歌单操作增强**:
  - **全列表快捷收藏**: 搜索结果、收藏歌曲、歌单详情、排行榜、本地音乐、缓存与下载任务、歌手及专辑歌曲等位置均可直接添加到歌单。
  - **加入状态预检查**: 打开歌单选择器时先检查歌曲是否已存在；未加入的歌单保持默认绿色样式，已存在的歌单显示红色对号，添加后保持当前列表页码。
  - **歌单导出**: 支持将默认列表、收藏列表及自定义歌单导出为本地 JSON 文件。
- **列表分页与后台入口**:
  - **首页与末页快捷跳转**: 搜索、收藏、歌单广场、排行榜、本地音乐及歌手歌曲列表新增首页和末页按钮，并根据总页数更新可用状态。
  - **后台快捷菜单**: Web 播放器侧边栏新增后台管理入口。

### 🔧 修复与优化

- **自定义播放器路径下 PWA 添加应用失效与 404 问题修复 (#314)**:
  - **动态 `manifest.json` 生成与路径自适应**: 解决配置自定义 `PLAYER_PATH`（如 `/123` 等非默认路径）后，通过浏览器安装 PWA 应用时固定打开原路径导致报 `Not Found` 的问题。服务端动态拦截播放器 `manifest.json` 请求，自动根据当前生效的 `player.path` 适配 `start_url`、`scope` 及图标 `src`。
  - **前端 Manifest 引用相对化**: 将播放器 HTML 中硬编码的绝对路径 `<link rel="manifest" href="/music/manifest.json">` 改为相对路径 `manifest.json`，依托前端 `<base>` 标签动态解析，确保与当前部署路径严格对齐。
  - **Service Worker 缓存规则与预缓存完善**: 在 `sw.js` 预缓存清单中补全遗漏的核心脚本（`idb_store.js`、`common_ui.js`），并在请求拦截中排除 `/js/config.js` 和 `manifest.json`，避免服务端动态配置被持久化强缓存。

- **非管理员下载与缓存权限独立控制 (#284)**:
  - **权限判断机制调整**: 原先浏览器下载与服务器缓存是否可用由客户端设置"缓存歌曲文件"开关决定，现在改为通过后台系统配置独立控制，与客户端个人设置完全解耦，更加安全可靠。
  - **新增"开启非管理员浏览器下载"开关（默认开启）**: 关闭后，未登录管理员的公开用户及普通用户将无法直接使用浏览器下载歌曲，操作时会弹出管理员密码验证框。**已登录账号的用户下载不受此设置影响。**
  - **新增"开启非管理员服务器缓存"开关（默认关闭）**: 关闭后，未登录管理员的公开用户及普通用户将无法将歌曲缓存或写入服务器存储，操作时会弹出管理员密码验证框。**已登录账号的用户下载不受此设置影响。**
  - ![非管理员下载与缓存权限控制](md/log/2.0.1-1.png)
- **公开曲库本地播放修复**:
  - **公开用户目录路径匹配**: 修复在已登录个人/管理员账号时播放【公开歌曲】库 (`_open`) 中的文件，由于构建播放 URL 时未校验公开库视图状态而使用了个人账号路径 (`/api/music/cache/file/admin/...`) 导致返回 404，进而误触发在线换源重新解析的 Bug。
  - **全流程公开标识同步**: 修复公开曲库中单曲播放、浏览器下载、批量保存到设备、语音指纹识别、自动重映射以及批量分类等操作中缺失 `_open` 账号映射的问题，确保公开曲库下所有文件及播放操作均正确指向公开目录。
- **设置页面移动端适配优化**:
  - **滑动条布局响应式改造**: 将「全局透明度」、「歌词字体大小」等含滑动条的设置项从固定单行布局改为响应式两行布局——手机上标题独占一行、滑动条在下一行撑满宽度，电脑上保持左右并排，彻底解决手机端滑动条被严重压缩的问题。
  - **下拉选择器布局修复**: 「歌词字体」选择器与「读取」按钮、「播放失败策略优先级」等下拉项改用 `flex items-center justify-between`，手机和电脑上按钮均固定显示在下拉框右侧，不再单独折行。
  - **设置面板底部安全距离**: 系统、显示、逻辑三个设置 Tab 页底部新增移动端安全间距，防止最后一项内容被手机浏览器底部导航栏遮挡，确保「后台管理」等按钮可正常点击。
  - **弹出提示气泡防溢出**: 设置项问号图标的悬浮提示气泡新增视口边缘检测，显示时自动计算并修正横向偏移，防止气泡内容超出屏幕左右边缘显示不全。
- **同平台音源失效/死链自动剔除与深度替补 (#293)**:
  - **解决坏链无法尝试后续源的问题**: 修复了当某个自定义源脚本返回失效链接（如 403 防盗链、Socket Hang Up、连接超时或死链）导致无法播放时，系统误判解析成功而直接跨平台切换，未能尝试同平台其它已启用音源的缺陷。
  - **服务端排除式轮询 (`excludeApiSources`)**: 服务端 `callUserApiGetMusicUrl` 新增已失效自定义源排除参数，当客户端播放报错上报后，服务端自动剔除故障音源并顺位调用当前平台的下一个支持音源（例如酷狗从全豆要无缝切换至聆澜音源、独家音源）。
  - **播放恢复流层级解耦**: 客户端音频报错时优先在同平台内部穷尽所有备选自定义源；仅当当前平台所有可用源均确认无法播放后，才根据用户的「播放失败策略优先级」进入跨平台搜索或音质降级。
  - **设置说明精准化更新**: 更新了「自动尝试换源」与「播放失败策略优先级」的悬浮提示与选项说明，清晰界定同平台多源剔错轮询与跨平台换源策略的层级协作关系。
- **全平台音乐封面与歌手写真高清化升级**:
  - **腾讯音乐 (tx)**: 单曲、专辑、搜索结果、排行榜、歌单封面全量由 `R500x500` / `R300x300` 升级为官方最高支持的 `R800x800` 超清分辨率。
  - **酷狗音乐 (kg)**: 单曲封面、搜索、专辑大图、排行榜及歌单封面全面升级为 `1000x1000` 极清画质模板（`replace('{size}', '1000')`），告别以往低清模糊与马赛克（`240` / `400` / `480`）。
  - **酷我音乐 (kw)**: 格式化封面工具类及短链补全全面接入 `1000` 像素档位（默认 `formatPic(url, 1000)`），全量获取 1000px 级专辑高清水印图。
- **播放失败自动恢复**:
  - **异常暂停自动换源**: 修复解析到播放地址后播放器停留在“暂停中”且不继续换源的问题；播放异常暂停或地址加载失败时自动进入恢复流程。
  - **连续换源尝试**: 修复首次跨平台切换失败后恢复流程提前结束的问题，能够继续尝试尚未使用的可用平台。
- **歌曲添加位置配置修复**:
  - **底部添加配置生效**: 修复 `LIST_ADD_MUSIC_LOCATION_TYPE=bottom` 在 Web 播放器及服务端歌单接口中不生效的问题，未显式指定位置时统一读取用户配置。
- **歌曲删除与分页状态修复**:
  - **令牌认证兼容**: 修复已使用令牌登录但未保存明文密码时无法从列表删除歌曲的问题，并在认证失效后自动刷新令牌重试。
  - **保持当前页**: 添加或删除歌曲后刷新当前列表数据，不再自动跳回第一页。
