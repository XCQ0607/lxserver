...
- **非管理员下载与缓存权限独立控制 (#284)**:
  ...
  - **服务端双重防护**: 除前端 UI 拦截外，服务端的 `/api/music/cache/download` 与 `/api/music/cache/queue` 接口亦同步加入了权限校验，防止绕过前端（如通过 F12 篡改变量或直接发起 API 请求）实现越权缓存。
  - **播放器配置接口补全**: 将两项新开关同步至 `/api/music/config` 公共配置接口，确保播放器能正确获取服务端的真实权限状态并进行准确的 UI 判断。
